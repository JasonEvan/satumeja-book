...
export default function BookingConfirmationPage() {
    ...
    return (
      <main className="min-h-screen bg-cream px-5 py-16 text-center text-pine">
        <div className="mx-auto max-w-md rounded-3xl border border-gold bg-cream-2 p-8 shadow-sm">
          <h1 className="font-baloo text-2xl font-bold">
          ...
          <Link
            href="/"
            className="mt-6 inline-flex rounded-xl bg-pine px-5 py-3 font-bold text-cream-2"
          >
            Kembali ke booking
          </Link>
        </div>
  ...
  const whatsappMessage = [
    "Hai Satu Meja, saya ingin konfirmasi booking.",
    "",
    `Nama: ${confirmation.booking.customerName}`,
    `Waktu: ${formatDate(confirmation.booking.date)}, ${formatHour(confirmation.booking.startHour)} – ${formatHour(confirmation.booking.endHour)} WIB`,
    `Nomer Booking: ${confirmation.rentalId}`,
    "",
    "Terima kasih.",
  ].join("\n");
    ...
    <main className="min-h-screen bg-cream px-4 py-8 sm:py-12">
      <section className="mx-auto max-w-xl overflow-hidden rounded-[2rem] border-2 border-gold bg-cream-2 shadow-[0_20px_45px_-28px_rgba(27,58,43,0.6)]">
        <div className="bg-pine px-6 py-7 text-center text-cream-2 sm:px-10">
          ...
          <h1 className="mt-2 font-baloo text-3xl font-bold">
            Konfirmasi ke admin
          </h1>
          <p className="mt-2 text-sm leading-6 text-cream-2/85">
            Silakan konfirmasi booking ke admin dan kirim bukti pembayaran
            melalui WhatsApp.
          </p>
        </div>
        ...
        <div className="space-y-6 px-5 py-6 sm:px-10 sm:py-8">
          <div className="rounded-2xl border border-gold bg-[#fff7dd] p-4 text-center">
            ...
            <p className="mt-1 break-all font-mono text-sm font-bold text-pine">
              {confirmation.rentalId}
            </p>
            {expiryText ? (
          ...
          <div className="rounded-2xl border border-[#d8cfa9] bg-white p-4">
            <h2 className="font-baloo text-lg font-bold text-pine">
              Detail pesanan
            </h2>
            <dl className="mt-3 space-y-3 text-sm">
              ...
              <div className="flex justify-between gap-4">
                ...
                <dd className="text-right font-semibold text-ink">
                  {confirmation.booking.tableName}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-muted">Tanggal</dt>
                <dd className="text-right font-semibold text-ink">
                  {formatDate(confirmation.booking.date)}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-muted">Jam</dt>
                <dd className="text-right font-semibold text-ink">
                  {formatHour(confirmation.booking.startHour)} –{" "}
                  {formatHour(confirmation.booking.endHour)} WIB
                </dd>
              </div>
              <div className="flex justify-between gap-4 border-t border-dashed border-[#d8cfa9] pt-3">
                <dt className="font-bold text-pine">Total</dt>
                <dd className="font-baloo text-base font-bold text-red">
                  {formatRupiah(confirmation.booking.total)}
                </dd>
              </div>
            </dl>
          </div>
          ...
          <div className="qris-card !mb-0">
            <div className="qris-card__header">
              <div className="qris-card__copy">
                <p className="m-0 font-baloo text-[15px] font-bold text-pine">
                  Scan QRIS untuk Pembayaran
                </p>
                <p className="m-0 mt-1 text-[11px] text-muted">
                  Setelah membayar, kirim bukti pembayaran ke admin melalui
                  WhatsApp.
                </p>
              </div>
              <span className="rounded-full bg-pine px-3 py-1 text-[11px] font-bold uppercase tracking-[0.08em] text-cream-2">
                QRIS
              </span>
            </div>
