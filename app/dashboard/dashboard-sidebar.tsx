...
const links: Array<{
  ...
  {
    href: "/dashboard/pendapatan/rental",
    label: "Menu rental",
    description: "Performa per menu",
    icon: "rental",
  },
];
    ...
    <aside className="relative z-20 border-b border-white/10 bg-pine text-cream lg:sticky lg:top-0 lg:flex lg:h-screen lg:w-[17.5rem] lg:shrink-0 lg:flex-col lg:border-r lg:border-b-0">
      ...
      <div className="relative flex items-center justify-between px-5 py-4 lg:px-7 lg:pt-8 lg:pb-6">
        <Link className="flex items-center gap-3" href="/dashboard">
          ...
          <span>
            <span className="block font-baloo text-xl leading-none font-bold tracking-tight">
              Satu Meja
            </span>
            <span className="mt-1 hidden text-[10px] font-semibold tracking-[0.2em] text-cream/50 uppercase sm:block">
              Owner Console
            </span>
          </span>
        </Link>
        <div className="flex items-center gap-2 lg:hidden">
          <span className="rounded-full border border-gold/25 bg-gold/10 px-2.5 py-1 text-[10px] font-bold tracking-wider text-gold-soft uppercase">
            WIB
          </span>
          <button
            aria-label="Keluar"
      ...
      >
        {links.map((link) => {
          ...
          return (
            ...
            >
              <span
                className={`grid h-9 w-9 place-items-center rounded-xl transition-colors ${active ? "bg-pine text-gold-soft" : "bg-white/[0.07] text-cream/70 group-hover:text-gold-soft"}`}
              >
              ...
              <span>
                ...
                <span
                  className={`hidden text-[11px] lg:block ${active ? "text-muted" : "text-cream/35"}`}
                >
                  {link.description}
                </span>
              </span>
              {active && (
                <span className="absolute top-1/2 right-3 hidden h-1.5 w-1.5 -translate-y-1/2 rounded-full bg-gold lg:block" />
              )}
            </Link>
          );
        })}
      ...
      <div className="relative mt-auto hidden p-5 lg:block">
        <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-4 backdrop-blur-sm">
          <div className="mb-3 flex items-center gap-2 text-gold-soft">
            <span className="grid h-7 w-7 place-items-center rounded-lg bg-gold/10">
              <ClockIcon />
            </span>
          ...
          <p className="text-xs leading-5 text-cream/55">
            Seluruh waktu dan periode telah disesuaikan ke Indonesia Barat.
          </p>
        ...
        <button
          className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 py-2.5 text-xs font-bold text-cream/60 transition hover:bg-white/[0.07] hover:text-cream"
          onClick={handleLogout}
          type="button"
        >
          <ExitIcon />
