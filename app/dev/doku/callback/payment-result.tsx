...
export default async function DokuPaymentResult({
  outcome,
  searchParams,
}: PaymentResultProps) {
    ...
    <main className="doku-test-page">
      <section className="doku-test-card" aria-labelledby="doku-result-title">
        ...
        <p className="doku-test-intro">
          {isSuccess
            ? "Anda kembali dari proses pembayaran DOKU Sandbox."
            : "Anda membatalkan atau belum menyelesaikan proses pembayaran DOKU Sandbox."}
        </p>

        <dl className="doku-test-details">
          <div>
            <dt>Pesanan</dt>
            <dd>Booking test Satu Meja</dd>
          </div>
          <div>
            <dt>Invoice</dt>
            <dd>{invoiceNumber || "Tidak tersedia"}</dd>
          </div>
          <div>
            <dt>Total</dt>
            <dd>{amount}</dd>
        ...
        <p className="doku-test-note">
          Detail ini diteruskan dari sesi checkout untuk pengujian. Halaman
          redirect bukan bukti pembayaran final dan tidak mengubah database.
        </p>
