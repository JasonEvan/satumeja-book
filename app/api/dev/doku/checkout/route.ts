...
import { createDokuSandboxCheckout, DokuApiError } from "@/lib/doku";

export const runtime = "nodejs";

function getCallbackUrl() {
  const configuredUrl = process.env.DOKU_CALLBACK_URL;
...
export async function POST(request: Request) {
  try {
    ...
    const checkout = await createDokuSandboxCheckout({
      amount,
      customerName,
      customerEmail,
      customerPhone,
    ...
    if (error instanceof DokuApiError) {
      console.error("DOKU sandbox checkout rejected", {
